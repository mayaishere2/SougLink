...
void _deleteProduct(String productName) async {
  ...
  if (userId != null) {
    ...
    for (var doc in snapshot.docs) {
      await doc.reference.delete(); // Delete from Firestore
    }
  }
}

void _fetchSavedProducts() {
  String? userId = FirebaseAuth.instance.currentUser?.uid;
  ...
  if (userId != null) {
    _subscription = FirebaseFirestore.instance
      ...
      if (mounted) {
        setState(() {
          selectedProducts = snapshot.docs.map((doc) {
            return {
...
@override
void dispose() {
  _subscription?.cancel(); // Cancel Firestore subscription when widget is destroyed
  super.dispose();
}
                    ...
                    : () async {
                        ...
                        if (userId != null) {
                          ...
                          await FirebaseFirestore.instance
                              ...
                              .collection('products')
                              .add({
                            'name': availableProducts[index]['name'],
                            'image': availableProducts[index]['image'],
                            'timestamp': FieldValue.serverTimestamp(), // To track when added
                          });
                        }
  ...
  child: Column(
    ...
    children: [
      ...
      Row(
        ...
        onPressed: () {
          _deleteProduct(filteredProducts[index]['name']!);
        },
      ),
          Stack(
            children: [
